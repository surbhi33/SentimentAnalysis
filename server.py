...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        score_0_100=round((((score-(-1))*100)/2))
        t['color']=str(colors[score_0_100])
    return tweets
...
def tweets(name):
    ...
    median_ls = []
    for t in tweets:
        median_ls.append(t['score'])
    
    ret_temp=render_template('tweets.html', records=all_tweets, tweets=tweets, median=median(median_ls))
...
def following(name):
    ...
    """ 
    ls_users=fetch_following(api, name)
    ls_sorted= sorted(ls_users, key=lambda elem: elem['followers'], reverse=True)
    
    ret_temp_2=render_template('following.html', name=name, sorted_list=ls_sorted)
